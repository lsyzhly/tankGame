    ...
    if(b!=0)
    {
        ...
        else if(b->utype==myclock)
        {
            ...
            if(this->isPlayer==true)
            {
                ...
                addTimeFun(0,bind(ptr_fun(setTankState),false,true),150);
                std::string tempSound="sound/Fanfare.wav";
                GameSound(hwnd,tempSound);
                return bumpType::through;
            }
            else
            {
                ...
                addTimeFun(0,bind(ptr_fun(setTankState),true,true),150);
                std::string tempSound="sound/Fanfare.wav";
                GameSound(hwnd,tempSound);
                return bumpType::through;
            }
        ...
        else if(b->utype==cap)
        {
            ...
            addTimeFun(1,bind(ptr_fun(setCapTankState),this,false),150);
            std::string tempSound="sound/Fanfare.wav";
            GameSound(hwnd,tempSound);
            return bumpType::through;
        }
        else if(b->utype==shovel)
        {
            if(this->isPlayer==true)
            {
                ...
                GameSound(hwnd,tempSound);
                //setHqState(qiang);
                //addTimeFun(1,bind(ptr_fun(setHqState),tu),250);
    ...
    if(c)
    {
        if(this->t->isPlayer==true && c->isPlayer==true)
        {
            if(this->t!=c)
            {
                //add_to_delete(this,1);
                setTankState(true,false);
                addTimeFun(4,bind(ptr_fun(setTankState),true,true),100);
                std::string tempSound="sound/hit.wav";
                GameSound(hwnd,tempSound);
                return bumpType::abandonded;
            }
