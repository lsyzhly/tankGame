...
#include <d3dx9.h>
    ...
    //application title  
    const wstring APPTITLE = L"Create Surface Program";  
     ...
     **/  
	bool TankInit(wchar_t *f,LPDIRECT3DSURFACE9 *sur,int m,int n)
	{
		HRESULT result;
		for(int i=0;i<m;i++)
			for(int j=0;j<n;j++)
    ...
    {  
        ...
        {  
			...
			if(n==0){
				...
				recti.right = SCREENW/5+28;  
				recti.top = SCREENH/5;  
				recti.bottom = SCREENH/5+28; 
							rrr.left = 0;  
				rrr.right = 28;  
				rrr.top = 0;  
				rrr.bottom = 28; 
				...
				NULL,               //destination palette  
				NULL,               //destination rectangle  
				L"D:\\banana.bmp",     //source filename  
				&rrr,               //source rectangle  
				D3DX_DEFAULT,       //controls how image is filtered  
				0,                  //for transparency (0 for none)  
				NULL);
				if(!SUCCEEDED(resul)){
					OutputDebugString(L"wrong!\n");
				}
    ...
    {  
		...
		TankInit(L"D:\\player2.bmp",(LPDIRECT3DSURFACE9 *)player2,4,8);
